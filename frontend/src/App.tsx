...
function App() {
  ...
  const handleStartCollection = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:8000/api/data/start-collection",
        {
          method: "POST",
          headers: {
  ...
  const handleStopCollection = async () => {
    try {
      const response = await fetch(
        `http://localhost:8000/api/data/stop-collection?exchange=${selectedExchange}&symbol=${selectedSymbol}`,
        {
          method: "POST",
        }
      );
  ...
  const renderMainContent = () => {
    switch (activeView) {
      ...
      case "backtest":
        return (
          <BacktestPanel exchange={selectedExchange} symbol={selectedSymbol} />
        );
      case "analysis":
        return (
          <AnalysisPanel exchange={selectedExchange} symbol={selectedSymbol} />
        );
      case "data-management":
        return (
          <DataManagement exchange={selectedExchange} symbol={selectedSymbol} />
        );
      case "settings":
        return <SettingsPanel selectedExchange={selectedExchange} />;
      default:
        return (
          <DataPreview
            exchange={selectedExchange}
      ...
      >
        <div className="border-b px-6 py-4">
          <div className="flex items-center space-x-2">
            ...
            {sidebarOpen && (
              <h1 className="text-xl font-semibold">Bot Forge</h1>
            )}
          </div>
        </div>
                ...
                >
                  ...
                  <SelectContent>
                    {systemConfig?.supported_exchanges.map((exchange) => {
                      const exchangeInfo = SUPPORTED_EXCHANGES.find(
                        (e) => e.id === exchange
                      );
                      return (
