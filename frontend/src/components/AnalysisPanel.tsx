...
export default function AnalysisPanel({ exchange, symbol }: AnalysisPanelProps) {
  const [analysisType, setAnalysisType] = useState<string>('technical_indicators')
...
- **RSI**: Currently showing ${features[features.length - 1]?.rsi.toFixed(1)} (neutral territory)
- **MACD**: ${features[features.length - 1]?.macd > 0 ? 'Bullish' : 'Bearish'} signal detected
- **Bollinger Bands**: Price is ${features[features.length - 1]?.price > features[features.length - 1]?.bollinger_upper ? 'above upper band (overbought)' : features[features.length - 1]?.price < features[features.length - 1]?.bollinger_lower ? 'below lower band (oversold)' : 'within normal range'}

## Pattern Recognition
- Identified potential support level at ¥${(Math.min(...features.map(f => f.price)) + 500).toFixed(0)}
- Resistance level observed at ¥${(Math.max(...features.map(f => f.price)) - 500).toFixed(0)}
- Volume profile suggests ${Math.random() > 0.5 ? 'accumulation' : 'distribution'} phase

## ML Model Recommendations
- **Trend Prediction**: ${Math.random() > 0.5 ? 'Upward' : 'Sideways'} movement expected in next 24h
- **Volatility Forecast**: ${Math.random() > 0.5 ? 'High' : 'Low'} volatility anticipated
...
- Stop-loss suggestion: ${(features[features.length - 1]?.price * 0.95).toFixed(0)}
      `
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Select value={analysisType} onValueChange={setAnalysisType}>
                ...
                <SelectContent>
                  <SelectItem value="technical_indicators">Technical Indicators</SelectItem>
            ...
            <div className="space-y-2">
              ...
              <div className="flex space-x-2">
                <Button onClick={handleRunAnalysis} disabled={isAnalyzing} className="flex-1">
            ...
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={features.slice(-50)}>
                  ...
                  <Line type="monotone" dataKey="macd" stroke="#82ca9d" strokeWidth={2} name="MACD" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Trend Probability</CardTitle>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Export Analysis</CardTitle>
              <Download className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <Button onClick={handleExportAnalysis} size="sm" className="w-full">
