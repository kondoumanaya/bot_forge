...
export default function DataPreview({ exchange, symbol, timeframe, isCollecting }: DataPreviewProps) {
  ...
  const fetchTickData = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/data/tick?exchange=${exchange}&symbol=${symbol}&limit=50`)
      if (response.ok) {
  ...
  const fetchOrderBookData = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/data/orderbook?exchange=${exchange}&symbol=${symbol}&limit=1`)
      if (response.ok) {
        const data = await response.json()
        if (data.length > 0) {
  ...
  const chartData = tickData.slice(0, 20).reverse().map((tick, index) => ({
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Current bid/ask spread for {symbol}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    {orderBookData?.bids.slice(0, 10).map((bid, index) => (
                      <div key={index} className="flex justify-between text-sm">
                        <span className="text-green-600">¥{bid[0].toLocaleString()}</span>
                ...
                <ScrollArea className="h-[200px]">
                  <div className="space-y-1">
                    {orderBookData?.asks.slice(0, 10).map((ask, index) => (
                      <div key={index} className="flex justify-between text-sm">
                        <span className="text-red-600">¥{ask[0].toLocaleString()}</span>
        ...
        <CardContent>
          <ScrollArea className="h-[300px]">
            <div className="space-y-2">
              {tickData.map((tick) => (
                <div key={tick.id} className="flex items-center justify-between p-2 rounded-lg border">
                  <div className="flex items-center space-x-4">
                    <Badge variant={tick.side === 'buy' ? 'default' : 'secondary'}>
                      {tick.side.toUpperCase()}
                    </Badge>
                    <span className="font-mono">¥{tick.price.toLocaleString()}</span>
